...
| 方法     | 是否改原数组 | 返回值           | 常见用途         |
| -------- | ------------ | ---------------- | ---------------- |
| `slice`  | 否           | 提取的新数组     | 拷贝、截取展示   |
| `splice` | 是           | 被删除的元素数组 | 删除、替换、插入 |

#### `splice()`

从指定位置开始**删除任意数量元素**，并**可选地插入新元素**，**直接修改原数组**。
...
```javascript
arr.splice(2, 0, 'a', 'b'); // 从索引2开始，删除0个，插入两个新元素
// [0, 3, 'a', 'b', 4]
```
...
```javascript
arr.splice(1, 2, 'x', 'y'); // 删除索引1、2，插入'x','y'
// [0, 'x', 'y', 4]
```
...
```javascript
arr.splice(-2, 1); // 删除倒数第2个元素
```

只写一个参数：删除从该位置到末尾的所有元素

```javascript
arr.splice(2); // 删除索引2及后面的全部
```
