...
Spring Bean 的生命周期是一条清晰的管线，容器在每个阶段都会留出扩展点。理解它不仅是应对面试的关键，更是掌握 Spring 框架精髓的第一步。
...
- **一级缓存** `singletonObjects`：  
   存放的是已经完成生命周期、可直接使用的单例 Bean。
- **二级缓存** `earlySingletonObjects`：  
   存放的是“半成品” Bean，也就是已经实例化但还没完成依赖注入的对象。
- **三级缓存** `singletonFactories`：  
   存放的是一个可以返回“早期 Bean 引用”的工厂对象（一般用于 AOP 代理增强）。
...
- 这种机制 **仅适用于单例 Bean**，多例（`prototype`） Bean 无法通过这种方式解决。
- 如果在 Bean 创建过程中依赖链过长或出现构造器循环依赖，Spring 同样会抛异常。
- 如果你手动禁用了“循环引用”支持，也会直接报错。
