...
1. **第一次**：客户端向服务端发出连接请求，证明自己能发送。
2. **第二次**：服务端收到请求后，回应一个确认消息，证明自己既能接收也能发送。
3. **第三次**：客户端收到服务端的确认，再发出一次回应，最终确认自己也能接收。

这样三步走下来，双方都确认了彼此的发送和接收能力，连接才算正式建立。
...
| 构造方法                        | 返回值   | 说明                                                                                   |
| ------------------------------- | -------- | -------------------------------------------------------------------------------------- |
| `Socket(String host, int port)` | `Socket` | 根据指定的服务器 IP 和端口号，请求与服务端建立连接。连接成功后返回客户端 socket 对象。 |
...
| 构造方法                 | 返回值         | 说明                               |
| ------------------------ | -------------- | ---------------------------------- |
| `ServerSocket(int port)` | `ServerSocket` | 创建服务端 socket 并绑定指定端口。 |
...
在最基础的实现中，每有一个客户端连接，服务端都会调用 `accept()` 返回一个新的 `Socket` 对象。常见做法是为每个连接单独开启一个线程去处理。

但如果连接数量非常多，就会导致线程数暴涨，占用大量资源，最终可能导致服务器宕机。

解决方法是：

- 使用 **线程池** 来统一管理线程，避免无限创建新线程。
- 或者采用 **NIO（非阻塞 I/O）** 机制来优化高并发场景。
